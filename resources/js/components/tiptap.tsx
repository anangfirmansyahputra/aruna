...
const Tiptap = ({ content, setContent, editable = true }: TiptapProps) => {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: `prose prose-sm sm:prose-base lg:prose-lg w-full xl:prose-2xl outline-none rounded-md ${
          editable && 'min-h-[300px] border border-[#d9d9d9] p-4'
        }`,
      },
    },
    onUpdate: ({ editor }) => {
  ...
  return (
    <div className="w-full space-y-4">
      {editable && (
        <ToolbarEditor editor={editor} handleImageUpload={handleImageUpload} />
      )}
